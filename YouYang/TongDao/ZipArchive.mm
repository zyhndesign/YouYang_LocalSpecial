...
{
	_unzFile = zip_unzOpen( (const char*)[zipFile UTF8String] );
	if( _unzFile )
	{
		unz_global_info  globalInfo = {0};
		if( zip_unzGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
		{
			//NSLog([NSString stringWithFormat:@"%l entries in the zip file",globalInfo.number_entry] );
		}
	}
...
{
	BOOL success = YES;
	int ret = zip_unzGoToFirstFile( _unzFile );
	...
	do{
		if( [_password length]==0 )
			ret = zip_unzOpenCurrentFile( _unzFile );
		else
			ret = zip_unzOpenCurrentFilePassword( _unzFile, [_password cStringUsingEncoding:NSASCIIStringEncoding] );
		if( ret!=UNZ_OK )
		{
		...
		ret = zip_unzGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
		if( ret!=UNZ_OK )
		{
		...
		zip_unzGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
		...
		while( fp )
		{
			read=zip_unzReadCurrentFile(_unzFile, buffer, 4096);
			if( read > 0 )
			{
...
-(BOOL) UnzipCloseFile
{
	_password = nil;
	if( _unzFile )
		return zip_unzClose( _unzFile )==UNZ_OK;
	return YES;
}
